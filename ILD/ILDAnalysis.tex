...
One can derive from (\ref{formula:DiffSigma_3}) the total cross section, an observable, which can be expressed as
...
These models can have different impact on the left-handed or right handed couplings. 
...
The measurement of the $A_{FB}^b$ value at the SLC have 27\% higher uncertainty than LEP results~\cite{bib:SLC}.
...
The precise knowledge of the $Zb\bar{b}$ vertex at LEP~I allowed to put constraints on the top quark mass~\cite{bib:LEPTOP}.
...
This process involve $tbW$ vertex and therefore, its total cross section depends on weak coupling of the top quark. 
...
To measure the electroweak couplings of the top quark at hadron colliders, the associate production of top quark pair with $Z^0$ or $W^\pm$ bosons is required. 
...
\begin{figure}[h]
	{\centering
		...
		\caption{\sl The result of the two-dimensional best fit for ttW and ttZ cross sections (cross symbol) is shown along with its 68 and 95\% confidence level contours. The result of this fit is
			superimposed with the separate ttW and ttZ cross section measurements, and the corresponding
			1 standard deviation (1$\sigma$) bands, obtained from the dilepton, and the trilepton/four-lepton
			channels, respectively. The figure also shows the predictions from theory and the corresponding
			uncertainties.~\cite{bib:ILCTOP}.}
...
One of the main goals of this thesis is to increase purity and efficiency of the b-jet charge measurement using the ILD environment. 
