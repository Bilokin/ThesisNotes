...
\begin{itemize}
	\item The number of accepted events is improved by 7\%;
	\item The $A_{FB}$ reconstruction is improved before correction by 3\%;
	\item The vertex charge purity is 84\% after recovery using the data-driven measurement, as compared to 78\% before recovery;
	\item The kaon charge purity stays the same, but the number of kaons is increased; 
	\item The \textit{main improvement} of the vertex charge recovery is the almost constant charge purity in the barrel. region.
\end{itemize}
The data-driven charge purity measurement and the polar angle correction algorithm are proven to work with different vertexing algorithms. 
