...
Regarding the similarity of the distributions for two processes shown in Figures ~\ref{fig:GenVtx_3}, \ref{fig:GenHadronParams_3} and \ref{fig:GenVtxOffset_3}, the performance of the vertexing algorithm should be identical for the same energy and the direction of the b-hadron decays.
...
\end{equation}
\item The algorithm creates new reconstructed vertices with add selected prongs and links them to the given jet.
\end{itemize}
...
\begin{itemize}
\item The algorithm is not changing the categories of b-hadron prongs, which have no reconstructed tracks;
\item The reconstructed prongs with no assigned VXD or FTD hits are not used either;
...
This section of the thesis describes an application of the jet charge measurement technique, described in Sec.~\ref{sec:JetChargeReconstruction}, to the forward-backward asymmetry measurement for $e^+e^- \to t\bar{t}$ process, which is used to determine the electroweak couplings of the top quark.

The measurements of the top quark properties is one of the key points of the \sm\ physics program at the ILC.

The dominant decay mode of the top quark is $t\to bW$, which is 99.8\% of all top decays, the $W^\pm$ boson can decay to a lepton-neutrino pair or a quark pair creates a six fermion final state of the process.
Therefore, the $t\bar{t}$  decays are classified by the $W^\pm$ decay modes:
\begin{itemize}
	\item The fully hadronic decay $t\bar{t} \to bq\bar{q} \bar{b} q\bar{q}$ is 46.2\% of the events;
	\item The semileptonic decay $t\bar{t} \to bq\bar{q} \bar{b} l\nu_l$ is 43.5\% of the events;
	\item The fully leptonic decay $t\bar{t} \to b l^- \nu_{l^-} \bar{b} l^+\nu_{l^+}$ 10.3\% of the events;	
\end{itemize}
In this section of the thesis, the semileptonic $t\bar{t}$ decays are used because of the following reasons:
\begin{itemize}
	\item The semileptonic top pair decays have clear signature of two b-jets and isolated lepton from $W^\pm$ boson, which makes them easily reconstructable, contrary to the fully hadronic and fully leptonic decays;
	\item The results obtained via the jet charge technique application can be used to extend the previously developed studies~\cite{bib:ILCTOP}.
\end{itemize}
