...
This chapter concentrates only on the left-handed electron polarization case, because of lepton migration problem caused by the $W^\pm$ kinematics.
%The setup of the 
...
\end{itemize}
To increase the purity can apply the following cuts on the b-hadron kinematics in case of the vertex charge usage:
\begin{equation}
	b-tag > 0.8\text{ and } |p|_{had} > 25\text{\,GeV}.
\end{equation}
...
The combined b-jet charge measurement shows a precision on the asymmetry $A_{FB}^{rec}/A^{gen}_{FB} \approx 93\%$ with efficiency of 20\%.
