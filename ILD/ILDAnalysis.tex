...
This choice is motivated to use the b-jet charge measurement to recover lepton migration effect, described in~\cite{bib:ILCTOP}, it also allows for a direct comparison and combination of the new results with previous studies done using $W^\pm$ lepton charge method.
...
To avoid the charge migrations, one can use the compatible combinations of two or more charge signatures.
In semileptonic \ttbar\ event one has two b-jets with vertex and kaon charge signatures each and one $W^\pm$ lepton charge signature. The general charge combination rules are:
\begin{itemize}
...
In total one has 11 charge pair signatures. 
