...
\begin{equation}
	f(A,B,C) = A(1+\cos^2\theta) + B \cos\theta + C \sin^2\theta,
	\label{formula:PolarAngleFit_3}
\end{equation}
where $A$, $B$ and $C$ are the fitted parameters. 
...
Using the extructed polar angle function the reconstructed $A_{FB}^b$ value is calculated as
\begin{eqnarray}
	A_{FB} = \frac{3}{4}\frac{B}{2 A  + C}.
\end{eqnarray} 
...
\begin{figure}
	...
	\begin{subfigure}{0.5\textwidth}
		...
		\caption{\label{fig:BAsymmetryCorrectedRec_a_3} }
	\end{subfigure}% 
	\begin{subfigure}{0.5\textwidth}
		\centering
		\includegraphics[width=0.95\textwidth]{ILD/plots/basymmetry-rec-corr-nobkg-right.pdf}
		\caption{\label{fig:BAsymmetryCorrectedRec_b_3} }
	\end{subfigure}
	...
	\label{fig:BAsymmetryCorrectedRec_3}
\end{figure}
The main improvements be the recovery are the following:
\begin{itemize}
	\item The number of accepted events is improved by 9\%;
	\item The $A_{FB}$ reconstruction is improved before correction by 3\%;
	\item The vertex charge purity is 84\% after recovery using the data-driven measurement, as compared to 80\% before recovery;
	\item The kaon charge purity stays the same, but the number of kaons is increased by 10\%; 
	\item The vertex charge recovery provides close to constant charge purity in the barrel region of the ILD experiment, as can be seen in Fig.~\ref{fig:BAsymmetryRec_3}. This is the consequence of the recovery of the peaks of missing vertex particles as it is described in Sec.~\ref{sec:VtxRecovery}.
...
The data-driven charge purity measurement and the polar angle correction algorithm are proven to work with the different vertexing algorithms. 
...
THe knowledge of the charge purity from the reconstructed events, or future ILC data is essential for the present analysis. 
...
The charge purity $p$ is computed using expressions (\ref{formula:PurityA_3}) and (\ref{formula:PurityR_3}).
...
\end{itemize}
The kaon-vertex  charge measurement have higher value of purity than the vertex-vertex and kaon-kaon combinations. 
The reconstruction algorithms can by accident lose a particle from the reconstructed secondary or tertiary vertices. 
Thus, the true $B_0$ hadron will be reconstructed as a charged hadron.
The $B^0-\bar{B}^0$ oscillations can flip the charge of the $K$-meson, which will create an fake compatibility between the kaon and vertex charges.
...
The main advantage of this charge purity measurement procedure is its independence from generator information. 
...
To compute the correction, one defines the number of accepted events in the forward region $N_a^+$ and in the backward region $N_a^-$ 
\begin{equation}
\begin{array}{l}
...
The computation is possible for each bin of the b-quark polar angle histogram. 
The equation (\ref{formula:Correction_3}) is true for a range in polar angle and or for a bins in the histogram.
To simplify the procedure, the purity $p$ is calculated and averaged for all bins of the polar angle histogram. 
...
\end{figure}
The further improvement of the results is done using the vertex charge recovery. 
...
The ILC is able to provide a solution to the LEP tension of the $A_{FB}^b$ measurement with the \sm\ prediction.
The $A_{FB}$ values as a function of $\sqrt{s}$ along with the experimental results and ILC expected precision is shown in Fig.~\ref{fig:LEPILC_3}. The LEP measurement on the $Z^0$ pole is still the most precise b-quark $A_{FB}$ measurement. 
Using the $A_{FB}$ and total cross section expected precision.
\begin{figure}
	\centering
	\begin{subfigure}{0.5\textwidth}
		\includegraphics[width=0.95\textwidth]{ILD/plots/afb-sqrts.pdf}
		\caption{\label{fig:LEPILC_a_3} }
	\end{subfigure}% 
	\begin{subfigure}{0.5\textwidth}
		\centering
		\includegraphics[width=0.95\textwidth]{ILD/plots/sigma-sqrts.pdf}
		\caption{\label{fig:LEPILC_b_3} }
	\end{subfigure}
	\caption{\sl Tree level \sm\ forward-backward asymmetry (a) and total cross section prediction (b). Results of the low-energy experiments and LEP measurements are taken from~\cite{bib:LEPpuzzle}.}
	\label{fig:LEPILC_3}
\end{figure}
\begin{figure}
	\centering
	\begin{subfigure}{0.5\textwidth}
		\includegraphics[width=0.95\textwidth]{ILD/plots/lep-result.pdf}
		\caption{\label{fig:LEPILCResult_a_3} }
	\end{subfigure}% 
	\begin{subfigure}{0.5\textwidth}
		\centering
		\includegraphics[width=0.95\textwidth]{ILD/plots/ilc-result.pdf}
		\caption{\label{fig:LEPILCResult_b_3} }
	\end{subfigure}
	\caption{\sl Tree level 1\,$\sigma$ allowed regions defined by the forward-backward asymmetry and total cross section measurements at LEP (a) and ILC (b). }
...
Thus, the distributions, presented in Fig.~\ref{fig:BAsymmetryFinal_3}, are fitted by the common function
\begin{equation}
	F (\cos\theta) = A (1+\cos^2\theta) + B\cos\theta + C \sin^2\theta.
\end{equation}
to find the parameters $A$, $B$ and $C$, which depend on the b-quark form factors. 
