...
The clear process signature of two b-jets, two light jets and an isolated lepton makes the semileptonic decay mode easily reconstructable and usable for the forward-backward asymmetry $A_{FB}$ measurement. 
...
This chapter concentrates only on the left-handed electron polarization case, because of lepton migration problem caused by the $W^\pm$ kinematics. The main goal of the study is to increase method efficiency using new b-jet charge measurement methods, which should bring down the statistical uncertainties on the \ttbar Z$^0$ form factors and couplings.  
%The setup of the 
...
The prevoius studies demonstrated, that the purity of the full semileptonic \ttbar\ process selection is 91\% and final selection efficiency is 54\%~\cite{bib:ILCTOP}.
...
In this studies, the top polar angle is calculated from the hadronic decay products of the top quark.
...
Hence, the main challange of the study is to identify the charge of the reconstructed hadronic top or anti-top quark. 
...
All three basic methods, which are demonstrated in Fig.~\ref{fig:OneCharge_3}, have significant charge impurity, which cause a confusion of reconstructed top quark with anti-top quark.
These charge migrations result in the divergence between the generated polar angle distribution and reconstructed curves of all three methods. 
...
The quality of the top polar angle and forward-backward asymmetry reconstruction can be estimated by the ratio of the reconstructed $A_{FB}^{rec}$ over generated $A^{gen}_{FB}$.
...
On has the following charge combination rules to accept an event for the top polar angle and the $A_{FB}$ measurement:
\begin{itemize}
	...
	\item The  $W^\pm$ lepton charge should have same charge as the b-jet charges from hadronically decayed top quark.
	\item The $W^\pm$ lepton charge can be used alone after cuts on the reconstructed top $\chi^2_{t}$ or top Lorentz gamma factor $\gamma_{top}$.
\end{itemize}

Accoriging to these rules, one has the following charge pair signatures for a semileptonic \ttbar event:
\begin{itemize}
	...
	\item $W^\pm$ lepton charge after a $\gamma_{top}$ cut  ({\sc l }cut).
\end{itemize}
To increase the charge purity can apply the following cuts on the b-hadron kinematics in case of the vertex charge usage:
\begin{equation}
	btag > 0.8\text{ and } |p|_{had} > 35\text{\,GeV}.
\end{equation}

To prove the concept of the b-jet charge measurement, the first four methods were used to compute top polar angle distribution shown in Fig.~\ref{fig:TopAsymmetryNoCutBjet_3}, where one can see a good agreement between the generated and reconstructed top polar angle histograms. 
The combined b-jet charge measurement shows a good precision on the asymmetry $A_{FB}^{rec}/A^{gen}_{FB} \approx 93\%$ with total efficiency of 18\%.
\begin{figure}[h]
	{\centering
		\includegraphics[width=0.55\textwidth]{ILD/plots/top-asymmetry-bonly-nocut.pdf}
...
\end{figure}
One can purify the sample by changing the cuts on the b-hadron kinematics, but with a penalty on statistics of the sample. 

This result of the  b-jet charge measurement can be applied directly on the fully hadronic \ttbar\ decays for left-handed electron polarization, which will significantly increase the statistics. 

The vertex charge recovery increases the statistics by 10\% and the $A_{FB}^{rec}/A^{gen}_{FB}$ ratio by 5\%. 
However, the effects of the vertex charge recovery will be discussed in Section~\ref{sec:BBBarresults} for the $e^+e^-\tob\bar{b}$ process. 


%To increase the purity can apply cuts on the b-hadron kinematics in case of the vertex charge usage, or on the reconstructed top $\chi^2_{t}$ in case of the  $W^\pm$ lepton charge application.
