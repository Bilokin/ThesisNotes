...
One should pay attention to the statistical correlations between the observables, which influence the final precision on the form factors. The correlation factor between the left-handed parameters $A^L$ and $B^L$ is $\rho^L_{AB} = 0.86$, while for the right-handed parameters $\rho^R_{AB} = 0.3$.
In general, the correlation factor value proportional to the $A_{FB}$ as $\rho_{AB} \approx 1.2 A_{FB}$.
