...
The results for all possible combinations of the vertex and kaon charges for b-quark polar angle reconstruction is demonstrated in Fig.~\ref{fig:BAsymmetryFirst_3}. The difference in the generated b-quark polar angle distributions are caused by the different the $Z^0$ boson couplings to the left-handed and the right-handed fermions. 
...
The forward inefficiency problem makes impossible the measurement of the $A_{FB}^b$ using a simple counting method. This problem can be avoided by the fitting the unaffected central bins with a function and extrapolating the fitted function on full polar angle range $(-1,1)$. However, the sensitivity of the current ILD experiment configuration to New Physics effects is lost for $|\cos\theta| > 0.85$.
...
\subsubsection{Recovery influence}
