...
The $W^\pm$ boson kinematics strongly depends on the polarization of the initial state:
...
The main goal of the study is to increase method efficiency using new b-jet charge measurement methods, which should bring down the statistical uncertainties on the \ttbar Z$^0$ form factors and couplings.  
...
One can reuse the kinematical cuts to suppress background processes from~\cite{bib:Doublet}[Jeremy] defined in Table~\ref{table:ttbarselection}
...
In this studies, the top polar angle is be calculated  of the top or anti-top quark, which decayed hadronically.
...
All three basic methods, which are demonstrated in Fig.~\ref{fig:OneCharge_3}, have significant charge impurity, which cause a confusion of reconstructed top quark with an anti-top quark.
...
The most used method is the vertex charge and kaon charge combination from the same b-jet, as can be seen from Fig.~\ref{fig:TopAsymmetryNoCutBjet_b_3}, which is due to the chosen cuts on the b-tag. 
\begin{figure}[h]
	\centering
	\begin{subfigure}{0.5\textwidth}
...
The vertex charge recovery increases the statistics by 7\% and the $A_{FB}^{rec}/A^{gen}_{FB}$ ratio by 4\%. 
...
\begin{equation}
	\chi^2_{top} = (\frac{\gamma_t-1.435}{\sigma_{\gamma_{t}}})^2 + (\frac{p^*_b-60}{\sigma_{\gamma_{p^*_b}}})^2 + (\frac{\cos\theta_{bW} - 0.23}{\sigma_{\cos\theta_{bW}}})^2,
\end{equation}
where $p^*_b$ and $\cos\theta_{bW}$ have been used in Eq.~\ref{formula:Chi2Top_3} and Lorentz factor $\gamma_t=E_{t}/m_{t}$ of the reconstructed top quark decayed hadronically. The almost all events with $\chi^2_{top} < 15$ have correct combination of a b-jet with hadronic $W^\pm$, which propagated into a correct interpretation of the $W^\pm$ lepton charge.
The application of all possible pair combination of the vertex charge, kaon charge and $W^\pm$ lepton charge to the top polar angle reconstruction can be seen in Fig.~\ref{fig:TopAsymmetryChi_a_3}. 
...
This method allows to reconstruct the top forward-backward asymmetry with $A_{FB}^{rec}/A^{gen}_{FB} = 93\%$ with the final efficiency of 39\%, which improves the published result~\cite{bib:ILCTOP} by 28\%. 
