...
The mass of the top quark is comparable to the electroweak symmetry breaking scale. 
This fact makes the top quark a subject of many New Physics theories. 
The measurements of the bottom quark properties, the partner of the top quark, have revealed a tension with \sm\ predictions. 
...
The beams of the $e^+e^-$ colliders, like ILC, will not be fully polarized, therefore one should take into account the electron beam polarization degree $\mathcal{P}$ and positron polarization degree $\mathcal{P}'$, therefore, the expression for the total cross section is
\begin{equation}
...
The TeVatron, which has proton-antiproton beams, have high production rate of the single top-quark via $q\bar{q}' \to W^\pm \to t\bar{b}$ process. %at a higher rate than proton-proton machines. 
...
The forward-backward asymmetry at TeVatron for strong interaction vertex $gt\bar{t}$ has been measured to be consistent with the \sm\ expectations~\cite{bib:TeVAfb}. 
...
The possible $Z^0$\bbbar associate production study at the LHC can provide only the cross section magnitude value, which has been measured at LEP~I to have no tension with the \sm\ prediction. 
...
The acceleration technologies available today allow for the construction and the running of a linear electron-positron collider at center-of-mass energies well above top pair production threshold. The precision of the linear colliders allows detection and reconstruction of all \sm\ decay modes of the top quark: fully leptonic, semileptonic and fully hadronic decays. 
...
The main advantage of the combination with the b-jet charge is that this method do not require precise knowledge of the top quark decay kinematics, as it is needed for the $\chi^2_{top}$ cut method. 
...
The first attempt to use the jet charge technique in fully hadronic \ttbar\ decays was done by~\cite{amjad:tel-00949818}, but it required large simulation-dependent corrections. 
...
An information about the bottom jet charge is useful for all \sm\ processes, where b-jets appear as decay products. 
...
This requires a high purity and a high efficiency of the b-jet charge reconstruction, which is an ultimate challenge for every reconstruction algorithm and all subdetectors of the experiment. 

One of the main goals of this thesis is to develop a method of the b-jet charge measurement with high purity and efficiency using the full simulation of the ILD experiment. 
...
The most relevant standard reconstruction algorithms of the {\sc ilcsoft} for the jet charge measurement are described below:
\begin{itemize}
...
The jet charge measurement is a completely independent reconstruction tool, which allows to study the top decay kinematics and to control the systematics effects.

The jet charge measurement is indispensable in case of the fully hadronic \ttbar\ decays, where there is no other sources of information about the top quark charge. 
...
The relative precision on the b-quark form factors is shown in Table~\ref{table:fffinal}. 
